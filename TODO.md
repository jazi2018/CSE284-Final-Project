...
5. ~~Implement basic FLARE computational tricks~~
    ...
    - Probabilities explained in supplementary section
        - Note: in my implementation since I'm still not accounting for genetic distance (yet),
        the probabilities are still relatively simple. But the transition probabilities should be
        O(n) now instead of O(n^2)
